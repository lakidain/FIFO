...
#define FIFO_NAME1 "escrituraLimpieza"
...
int main(int argc, char* argv[])
{
    ...
    srand(9695054);
    while(1){  //hacemos un for con el numero de operaciones que vamos a realizar
        sleep(1);        
        ...
        if ((num = write(fq, robot, strlen(robot))) == -1)  //iniciamos la operacion de escritura, esto equivale a pedir permiso a la controladora para que atienda la peticion
            ...
            if ((num = read(fw, s, 3)) == -1)   //si la peticion se ha realizado correctamente esperamos la confirmacion por parte de la controladora
                ...
                random = rand()%3+1;    //generamos un random para decidir cuanto tiempo va a tardar en realizar el trabajo
                ...
                if ((num = write(fq, "FIL", 3)) == -1){ //enviamos a la controladora el aviso de que hemos finalizado de trabajar
	                perror("write");              
                }
            }
        }
