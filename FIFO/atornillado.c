...
#define FIFO_NAME0 "lecturaControlador" //declaramos las FIFO necesarias
#define FIFO_NAME1 "escrituraAtornillado"

#define robot "ATO" //nombre del robot, esta cadena la vamos a usar para identificarlo en la controladora
#define NUMOPERACIONES 5   //operaciones totales que queremos realizar

int main(int argc, char* argv[])
{
    printf("%s... OK\n",robot); //Hacemos un ok de que el robot ha llegado
    char s[2];
    int num, fq,fw;
    ...
    printf("%s: inicio jornada (PID %d)\n",robot,getpid()); //tras hacer todas las inicializaciones el robot esta listo para empezar a realizar su jornada

    srand(1029380); //esto es una inicializacion random con una seed, internamente nos sirve para que al hacer rand() salgan numeros distintos entre diferentes procesos, si no tendrian la misma semilla y saldria en todos lo mismo
